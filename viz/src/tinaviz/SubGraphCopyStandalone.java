...
public class SubGraphCopyStandalone extends NodeFilter {
    ...
    public NodeList preProcessing(Session session, View localView, NodeList input) {
        ...
        if (debugMeso)System.out.println("source=" + source);
        ...
        if (sourceView == null) {
           if (debugMeso) System.out.println("uh oh! i am a source and my 'source' parameter is totally wrong! got " + source);
            return output;
        }

        String category = (String) localView.properties.get(root + KEY_CATEGORY);
       if (debugMeso) System.out.println("read KEY_CATEGORY to " + category);
        if (category == null) {
        ...
        if (sourceView.getGraph().size() < 1) {
           if (debugMeso) System.out.println("original graph is zero-sized.. ");
            return output;
        }
       if (debugMeso) System.out.println("MESO current view size: " + localView.getGraph().size());

        if (debugMeso)System.out.println("MESO category: " + category);
        if (debugMeso)System.out.println("MESO oldCategory: " + oldCategory);
        ...
        // try to change the selection changed predicate by looking at selection
        if (!selectionChanged) {
            // part 1/2

            for (Entry<Integer, Node> e : sourceView.getGraph().storedNodes.entrySet()) {
                if (e.getValue().selected) {
                    if( ! selection.containsKey( e.getValue().id) )  {
            ...
            // part 2/2
            for (Integer i : selection.keys().elements()) {
                for (Entry<Integer, Node> e : sourceView.getGraph().storedNodes.entrySet()) {
                    if (e.getValue().id==i && !e.getValue().selected) {
                            selectionChanged = true;
                            break;
                    }
                }
        ...
        if (selectionChanged) {
            ...
            if (debugMeso)System.out.println("MESO something (item or category) changed, updating subgraph copy....");
            NodeList newNodes = new NodeList();

            for (Entry<Integer, Node> e : sourceView.getGraph().storedNodes.entrySet()) {
                System.out.println("copying node from macro to meso");
                if (e.getValue().selected) {

                    System.out.println("the node is selected, we add it as root");
           ...
           if (debugMeso) System.out.println("MESO newNodes size: " + newNodes.size());

            localView.getGraph().clear();
            localView.updateFromNodeList(newNodes);
            if (debugMeso)System.out.println("MESO localView size: " + localView.getGraph().size());
            output = new NodeList(localView.getGraph().getNodeListCopy());
            if (debugMeso)System.out.println("MESO output size: " + output.size());
