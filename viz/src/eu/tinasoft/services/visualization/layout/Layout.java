...
public class Layout {
 ...
 public void macroViewLayout_TinaForce_backup2(View v, NodeList nodes) {
        ...
        for (Node n1 : nodes.nodes) {
            ...
            for (Node n2 : nodes.nodes) {
                ...
                boolean nodeisAloneAndFar = (n2_degree <= 1 && dist > Output.RADIUS_MAX * 5.0f);
    ...
    public void macroViewLayout_TinaForce_backup(View v, NodeList nodes) {
        ...
        float cooling = (0.05f / PApplet.sqrt(PApplet.sqrt(metrics.nbEdges + 1f)));

        float vlimit_max = 100f;

        float decay = PApplet.min(PApplet.exp(-v.layoutIterationCount * cooling), PApplet.exp(-v.layoutIterationCount * 0.005f));
        //float decay_slow = PApplet.min(PApplet.exp(-v.layoutIterationCount * cooling),PApplet.exp(-v.layoutIterationCount * 0.05f));
        ...
        //System.out.println("layout iter " + v.layoutIterationCount);

        if (true && Math.random() < 0.5f) {
            //System.out.println("decay: " + decay + " = PApplet.exp(-" + v.layoutIterationCount + " * " + cooling + ")");
            // System.out.println("decay_slow: " + decay_slow);
            //System.out.println("gravity: " + gravity);
        }
        ...
        for (Node n1 : nodes.nodes) {
            ...
            for (Node n2 : nodes.nodes) {
                ...
                if (v.layoutIterationCount == 1) {
                    //System.out.println("initializing layout..");
                    float theta = 2 * PApplet.PI * loopCounter;
                    dix = ((PApplet.cos(theta) - PApplet.sin(theta))) * 10f;
                    diy = ((PApplet.cos(theta) + PApplet.sin(theta))) * 10f;
                    ...
                    boolean nodeisAloneAndFar = (n2_degree <= 2 && dist > Output.RADIUS_MIN * 5.0f);
        ...
        if ((float) Math.random() < 0.01f) {
            System.out.println("checking for bary center");
            PVector oldBaryCenter = metrics.baryCenter;
            PVector newBaryCenter = nodes.computeMetrics().baryCenter;

            float threshold = 0.0001f;
            if (PVector.dist(oldBaryCenter, newBaryCenter) < threshold) {
                System.out.println("movement is small, pausing!");
                v.paused = true;
            }
